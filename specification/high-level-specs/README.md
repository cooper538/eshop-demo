...
| Service | Type | Database | Description |
|---------|------|----------|-------------|
| API Gateway | ASP.NET Core Web API | – | YARP reverse proxy, rate limiting |
| Product Service | ASP.NET Core Web API + gRPC | PostgreSQL | Product catalog, inventory management |
| Order Service | ASP.NET Core Web API + gRPC | PostgreSQL | Order lifecycle, Internal gRPC API |
| Notification Service | Worker Service | PostgreSQL | Sending notifications (simulated) |
| Analytics Service | Worker Service | – | Event analytics processing |
| Database Migration | Worker Service | – | Centralized database migrations |

**Note:** Ports are dynamically assigned by Aspire during development. See AppHost for service configuration.
...
- **Order/Product → Analytics**: Integration Events (asynchronous, MassTransit/RabbitMQ)
...
| RPC | Grpc.AspNetCore | 2.76.x |
| Messaging | MassTransit | 8.4.x |
...
| Resilience | Microsoft.Extensions.Http.Resilience | 10.x |
| Mapping | Riok.Mapperly | 4.x |
...
| Resilience | Standard resilience policies, gRPC deadlines |
...
| External API Integration | Typed HttpClient, resilience policies (SendGrid) |
| Architecture Testing | NetArchTest for layer dependency validation |
...
│   │   ├── EShop.Common.Api/       # HTTP/gRPC middleware, interceptors
│   │   ├── EShop.Common.Application/  # CQRS, behaviors, exceptions
│   │   ├── EShop.Common.Infrastructure/  # Data access utilities
│   │   ├── EShop.RoslynAnalyzers/  # Custom code analyzers
...
- **Common split into layers** - Api, Application, Infrastructure for clean separation
...
# API Gateway: Available via Aspire dashboard endpoints
...
When scaling to multiple teams/repos, shared projects can be extracted to internal NuGet packages. See [Shared Projects Spec - NuGet Extension](./shared-projects.md#6-future-extension-nuget-packages) for detailed migration path.
